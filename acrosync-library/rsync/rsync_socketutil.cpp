...
int SocketUtil::create(const char *host, int port, std::stringstream *error)
{
    ...
    struct addrinfo *info = 0;//为什么不是NULL
	int result = getaddrinfo(host, "873", &hints, &info);//change to 22 or 873
    if (result != 0) {
...
#if defined(WIN32) || defined(__MINGW32__)
    u_long iMode=1;
    result = ioctlsocket(sock, FIONBIO, &iMode);//The ioctlsocket function controls the I/O mode of a socket.block（0） or nonblock（1）FIONBIO阻塞模式
    if (result != NO_ERROR) {
	...
	if (GetLastError()) {
		*error << "faied to connect :" << GetLastError() << "--" << result;
		//close(sock);
		//return -1;
	}
	//io模式的设置，监视文件描述符的变化

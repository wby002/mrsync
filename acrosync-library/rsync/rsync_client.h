...
public:
	...
	// the remote directory 'path' (not recursively).  Each item will be returned by invoking the callback
	...
	void getRemoteFiles(const char* remoteTop, std::vector<Entry*>*remoteFiles);
...
private:
	...
	std::vector<Entry*> d_remoteFiles;
